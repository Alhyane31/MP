...
const HistoryScreen = () => {
  ...
  return (
    ...
    <View style={{ alignItems: 'center',  marginBottom: 0 , padding : 5}}>
              <Text style={{ fontSize: 22, textAlign: 'center',fontWeight: 'bold', color: 'white' }}> الأمراض</Text>
            </View>
      <FlatList
        ...
        renderItem={({ item }) => (
          <View style={styles.listItem}>
            <TouchableOpacity onPress={() => router.push({ pathname: '/agentsar', params: {  pathologyType: item.ID ,PathologyLabelFR: item.LibelleFR,PathologyLabelAR: item.LibelleAR}})}>
              <FontAwesome name="info" size={28} color="black" />
            </TouchableOpacity>
            <View style={styles.textContainer}>
              <Text style={[styles.name , { textAlign: 'right', fontSize: 16 }]}>{item.LibelleAR}</Text>
              <Text style={[styles.info, { textAlign: 'right', fontWeight: 'bold'  }]}> {new Date(item.History).toISOString().replace('T', ' ').slice(0, 19)}</Text>
            </View>
      ...
      />
       <View style={{ alignItems: 'center',  marginBottom: 0 , padding : 5}}>
              <Text style={{ fontSize: 22, textAlign: 'center',fontWeight: 'bold', color: 'white' }}>العوامل الممرضة</Text>
            </View>
      <FlatList
        contentContainerStyle={styles.listContainer}
        data={Agents}
